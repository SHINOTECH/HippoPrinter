...
stl_write_quad_object(stl_file *stl, char *file) {
	...
	for (i = 0; i < stl->stats.number_of_facets; i++) {
		...
		fprintf(fp, "%lf %lf %lf    %1.1f %1.1f %1.1f 1\n",
...
// 日期：2016/12/20 
// 参数: stl_file * stl
// 参数: const char * file，STL文件名
// 参数: const char * label，标签
// 返回: void
// 功能:将STL以ASCII格式写入到文件中
//************************************
void
stl_write_ascii(stl_file *stl, const char *file, const char *label) {
  int       i;
